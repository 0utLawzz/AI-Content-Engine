...
import { Download, Box, Video, Search, FileVideo, Layers, ArrowRight } from "lucide-react";
...
export default function Exports() {
  ...
  function getPlatformIcon(platform: string) {
    // We can't use complex SVGs or external icons easily, so we use standardized Lucide icons
      ...
      {(exportsLoading || projectsLoading) ? (
              ...
              <Card key={exp.id} className="bg-card/50 border-border/50 overflow-hidden flex flex-col group">
                <CardContent className="p-0 flex-1 flex flex-col">
                  <div className="p-4 border-b border-border/50 flex justify-between items-start bg-accent/20">
                    ...
                    <Badge variant={exp.status === 'completed' ? 'default' : 'secondary'} className={
                      exp.status === 'completed' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 
                      exp.status === 'processing' ? 'bg-primary/10 text-primary border-primary/20 animate-pulse' :
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    ...
                    <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground">
                      <span>{formatBytes(exp.fileSize)}</span>
                  ...
                  <div className="p-3 bg-accent/30 flex gap-2">
                    <Button 
                      ...
                      onClick={() => exp.fileUrl && window.open(exp.fileUrl, '_blank')}
                    >
                      <Download className="w-4 h-4" />
                      {exp.status === 'completed' ? 'Download File' : 'Not Ready'}
                    </Button>
                  </div>
                </CardContent>
              </Card>
        ...
        <div className="text-center py-24 bg-card/30 rounded-xl border border-dashed border-border/50">
          ...
          <p className="text-muted-foreground max-w-sm mx-auto">
            {search ? "No exports match your search." : "Rendered videos will appear here across all your projects."}
          </p>
        </div>
      )}
    </div>
